...
* Linear regression: $\newline$
  ...
  - $\textbf{Step 2}$: $\newline$
    ...
    *Note*:\newline
    ...
    iff the $\texttt{rank}(\mathbf{X^T X})=\texttt{rank}(\mathbf{X})$ is maximal 
    or if the the columns of the matrix $\mathbf{X}$ are linearly independent.\newline
    2. In the field of machine learning (ML), the vector $\mathbf{\beta}$ is split into 2 parts:
       the scalar $\texttt{b}:=\mathbf{\beta}_0$ ($\texttt{bias}$) and the 
       remainder of the vector $\mathbf{\beta}$, i.e. ($\texttt{w}$) also 
