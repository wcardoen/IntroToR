...
* use of the $\textcolor{blue}{\textbf{attributes()}}$ function :

The fundamental difference between an $\texttt{R}$ vector and matrix is content
presence of a $\textcolor{blue}{\textbf{dim}}$ attribute. 

to an $\texttt{R}$ matrix.
The $\textcolor{blue}{\textbf{dim}}$ attribute of an R vector is $\texttt{NULL}$.
...
We change easily convert a vector into matrix by setting the dimensions of the vector:
...
* Being in essence a vector, operations like $\textcolor{blue}{\textbf{*,/, +}}$
  happpen element-wise.
* There are also specialized functions:
  * $\textcolor{blue}{\textbf{rowMeans()}}$, $\textcolor{blue}{\textbf{colMeans()}}$: row and column means
  * $\textcolor{blue}{\textbf{\%*\%}}$: matrix multiplication
...
## Retrieving elements of matrices
